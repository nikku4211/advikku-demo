...
static inline void cell_song_hw_write_sfx_ch1(const uint16_t * inst) {
	
	//channel 1
	if (ch_sfx[0].queue_note) {
		//write instrument
		if (ch_sfx[0].queue_instrument) {
			if (ch_sfx[0].queue_volume) {
				if ((ch_sfx[0].volume & 0xf0))
					REG_SND1CNT = ((inst[ch_sfx[0].instrument] & 0x00ff) | (ch_sfx[0].volume << 12)) | ((ch_sfx[0].volume & 0xf0) << 4);
				else
					REG_SND1CNT = ((inst[ch_sfx[0].instrument] & 0x0fff) | (ch_sfx[0].volume << 12));
				if (!advik_global_command_linger)
					ch_sfx[0].queue_volume = 0;
			} else {
				REG_SND1CNT = inst[ch_sfx[0].instrument];
			}
			ch_sfx[0].queue_instrument = 0;
		}
		//write note
		if (inst[ch_sfx[0].instrument] & SSQR_LEN_MASK)
			REG_SND1FREQ = SFREQ_RESET | ch_sfx[0].current_freq | SFREQ_TIMED;
		else
			REG_SND1FREQ = SFREQ_RESET | ch_sfx[0].current_freq;
		ch_sfx[0].queue_note = 0;
	} else if (ch_sfx[0].queue_cut) {
		//cut the note through the instrument
		REG_SND1CNT = 0;
		ch_sfx[0].queue_cut = 0;
	} else {
		//do not retrigger the note otherwise
		REG_SND1FREQ = ch_sfx[0].current_freq;
	}
	
	if (ch_sfx[0].queue_timbre) {
		REG_SND1CNT = ((inst[ch_sfx[0].instrument] & 0xff00) | ch_sfx[0].timbre_goal);
	}
}

static inline void cell_song_hw_write_sfx_ch2(const uint16_t * inst) {	
	//channel 2
	if (ch_sfx[1].queue_note) {
		//write instrument
		if (ch_sfx[1].queue_instrument) {
			if (ch_sfx[1].queue_volume) {
				if ((ch_sfx[1].volume & 0xf0))
					REG_SND2CNT = ((inst[ch_sfx[1].instrument] & 0x00ff) | (ch_sfx[1].volume << 12)) | ((ch_sfx[1].volume & 0xf0) << 4);
				else
					REG_SND2CNT = ((inst[ch_sfx[1].instrument] & 0x0fff) | (ch_sfx[1].volume << 12));
				if (!advik_global_command_linger)
					ch_sfx[1].queue_volume = 0;
			} else {
				REG_SND2CNT = inst[ch_sfx[1].instrument];
			}
			ch_sfx[1].queue_instrument = 0;
		}
		//write note
		if (inst[ch_sfx[1].instrument] & SSQR_LEN_MASK)
			REG_SND2FREQ = SFREQ_RESET | ch_sfx[1].current_freq | SFREQ_TIMED;
		else
			REG_SND2FREQ = SFREQ_RESET | ch_sfx[1].current_freq;
		ch_sfx[1].queue_note = 0;
	} else if (ch_sfx[1].queue_cut) {
		//cut the note through the instrument
		REG_SND2CNT = 0;
		ch_sfx[1].queue_cut = 0;
	} else {
		//do not retrigger the note otherwise
		REG_SND2FREQ = ch_sfx[1].current_freq;
	}
	
	if (ch_sfx[1].queue_timbre) {
		REG_SND2CNT = ((inst[ch_sfx[1].instrument] & 0xff00) | ch_sfx[1].timbre_goal);
	}
}
static inline void cell_song_hw_write_sfx_ch3(const uint16_t * inst, const uint16_t * const * wave) {
	//channel 3
	if (ch_sfx[2].queue_note) {
		//write instrument
		if (ch_sfx[2].queue_instrument) {
			if (ch_sfx[2].queue_volume) {
				//check volume and assign the right value
				switch(ch_sfx[2].volume & 0x0c){
					case 0:
						REG_SND3CNT = SWAV_VOL0;
						break;
					case 4:
				...
				if (!advik_global_command_linger)
					ch_sfx[2].queue_volume = 0;
			} else {
				REG_SND3CNT = SWAV_VOL100;
			}
			//copy channel 3 waveform over
			REG_SND3SEL = 0;
			memcpy((void *)REG_WAVE_RAM, wave[ch_sfx[2].instrument], 16);
			REG_SND3SEL = 0xC0;
			
			ch_sfx[2].queue_instrument = 0;
		}
		//write note
		if (inst[ch_sfx[2].instrument << 1] == SWAV_LEN_ON)
			REG_SND3FREQ = SFREQ_RESET | ch_sfx[2].current_freq | SFREQ_TIMED;
		else
			REG_SND3FREQ = SFREQ_RESET | ch_sfx[2].current_freq;
		ch_sfx[2].queue_note = 0;
	} else if (ch_sfx[2].queue_cut) {
		//cut the note through the instrument
		REG_SND3CNT = 0;
		ch_sfx[2].queue_cut = 0;
	} else {
		//do not retrigger the note otherwise
		REG_SND3FREQ = ch_sfx[2].current_freq;
	}
	
	if (ch_sfx[2].queue_timbre) {
		REG_SND3SEL = 0;
		memcpy((void *)REG_WAVE_RAM, wave[ch_sfx[2].timbre_goal], 16);
		REG_SND3SEL = 0xC0;
	}
}
static inline void cell_song_hw_write_sfx_ch4(const uint16_t * inst) {
	//channel 4
	if (ch_sfx[3].queue_note) {
		//write instrument
		if (ch_sfx[3].queue_instrument) {
			if (ch_sfx[3].queue_volume) {
				if ((ch_sfx[3].volume & 0xf0))
					REG_SND4CNT = (((inst[ch_sfx[3].instrument] & 0x00ff) & SNOI_EXCLUDE_PACK) | (ch_sfx[3].volume << 12)) | ((ch_sfx[3].volume & 0xf0) << 4);
				else
					REG_SND4CNT = (((inst[ch_sfx[3].instrument] & 0x0fff) & SNOI_EXCLUDE_PACK) | (ch_sfx[3].volume << 12));
				if (!advik_global_command_linger)
					ch_sfx[3].queue_volume = 0;
			} else {
				REG_SND4CNT = (inst[ch_sfx[3].instrument] & SNOI_EXCLUDE_PACK);
			}
			ch_sfx[3].queue_instrument = 0;
		}
		//write note
		if (inst[ch_sfx[3].instrument] & SSQR_LEN_MASK)
			REG_SND4FREQ = SFREQ_RESET | (ch_sfx[3].current_freq & SNOI_EXCLUDE_UNPACK) | (SNOI_UNPACK(inst[ch_sfx[3].instrument] & SNOI_SHORT_PACK)) | SFREQ_TIMED;
		else
			REG_SND4FREQ = SFREQ_RESET | (ch_sfx[3].current_freq & SNOI_EXCLUDE_UNPACK) | (SNOI_UNPACK(inst[ch_sfx[3].instrument] & SNOI_SHORT_PACK));
		ch_sfx[3].queue_note = 0;
	} else if (ch_sfx[3].queue_cut) {
		//cut the note through the instrument
		REG_SND4CNT = 0;
		ch_sfx[3].queue_cut = 0;
	} else {
		//do not retrigger the note otherwise
		REG_SND4FREQ = (ch_sfx[3].current_freq & SNOI_EXCLUDE_UNPACK) | (SNOI_UNPACK(inst[ch_sfx[3].instrument] & SNOI_SHORT_PACK));
	}
	
	if (ch_sfx[3].queue_timbre) {
		REG_SND4CNT = ((inst[ch_sfx[3].instrument] & 0xff00) | ch_sfx[3].timbre_goal);
	}
}
static inline void cell_song_hw_write_sfx_ch5(const uint8_t * const * inst) {
	//channel 5
	if (ch_sfx[4].queue_note) {
		//write instrument
		if (ch_sfx[4].queue_instrument) {
			if (ch_sfx[4].queue_volume) {
				REG_SNDDSCNT = (REG_SNDDSCNT & 0xfffb) | ((ch_sfx[4].volume & 1) << 2);
				if (!advik_global_command_linger)
					ch_sfx[4].queue_volume = 0;
			} else {
				REG_SNDDSCNT = (REG_SNDDSCNT & 0xfffb) | ((0x01) << 2);
			}
			REG_DMA1CNT = 0;
			//set DMA transfer to DAC 1 FIFO
			REG_DMA1SAD = (uint32_t)inst[ch_sfx[4].instrument];
			REG_DMA1DAD = (uint32_t)&REG_FIFO_A;
			REG_DMA1CNT = DMA_DST_FIXED | DMA_REPEAT | DMA_32 | DMA_AT_REFRESH | DMA_ENABLE;
			ch_sfx[4].queue_instrument = 0;
		}
		//write note
		//set timer 0
		REG_TM0D = ch_sfx[4].current_freq;
		REG_TM0CNT = TM_ENABLE;
		ch_sfx[4].queue_note = 0;
	} else if (ch_sfx[4].queue_cut) {
		//cut the note through the instrument
		REG_DMA1CNT = 0;
		ch_sfx[4].queue_cut = 0;
	} else {
		//do not retrigger the note otherwise
		REG_TM0D = ch_sfx[4].current_freq;
		REG_TM0CNT = TM_ENABLE;
...
static inline void cell_song_hw_write_sfx_ch6(const uint8_t * const * inst) {
	//channel 6
	if (ch_sfx[5].queue_note) {
		//write instrument
		if (ch_sfx[5].queue_instrument) {
			if (ch_sfx[5].queue_volume) {
				REG_SNDDSCNT = (REG_SNDDSCNT & 0xfff7) | ((ch_sfx[5].volume & 1) << 3);
				if (!advik_global_command_linger)
					ch_sfx[5].queue_volume = 0;
			} else {
				REG_SNDDSCNT = (REG_SNDDSCNT & 0xfff7) | ((0x01) << 3);
			}
			REG_DMA2CNT = 0;
			//set DMA transfer to DAC 1 FIFO
			REG_DMA2SAD = (uint32_t)inst[ch_sfx[5].instrument];
			REG_DMA2DAD = (uint32_t)&REG_FIFO_B;
			REG_DMA2CNT = DMA_DST_FIXED | DMA_REPEAT | DMA_32 | DMA_AT_REFRESH | DMA_ENABLE;
			
			ch_sfx[5].queue_instrument = 0;
		}
		//write note
		//set timer 0
		REG_TM1D = ch_sfx[5].current_freq;
		REG_TM1CNT = TM_ENABLE;
		ch_sfx[5].queue_note = 0;
	} else if (ch_sfx[5].queue_cut) {
		//cut the note through the instrument
		REG_DMA2CNT = 0;
		ch_sfx[5].queue_cut = 0;
	} else {
		//do not retrigger the note otherwise
		REG_TM1D = ch_sfx[5].current_freq;
		REG_TM1CNT = TM_ENABLE;
...
void cell_sfx_update() {
	for (uint8_t i = 0; i < advik_global_channels; i++){
		if (advik_ch_sfx_ticks_per_row_counter[i] > 0) {
			
			advik_ch_sfx_ticks_per_row_counter[i]--;
			advik_ch_sfx_arp_tick[i]++;
			advik_ch_sfx_arp_index[i] = advik_ch_sfx_arp_tick[i] % 3;
			
			
			//handle effects for every tick but the first
				if (ch_sfx[i].delay_enabled) {
					if (ch_sfx[i].delay_ticks_counter > 0) {
						ch_sfx[i].delay_ticks_counter--;
					} else {
						if (advik_ch_sfx_order[i][advik_current-3] > 0x00){
							ch_sfx[i].instrument = advik_ch_sfx_order[i][advik_current-3] - 1;
							ch_sfx[i].queue_instrument = 1;
						}
						ch_sfx[i].base_note = advik_ch_sfx_order[i][advik_current-4];
						ch_sfx[i].current_note = ch_sfx[i].base_note;
						if (i < 3) {
							ch_sfx[i].base_freq = snd_freqs[ch_sfx[i].base_note];
							ch_sfx[i].current_freq = snd_freqs[ch_sfx[i].current_note];
						} else if (i >= 4) {
							ch_sfx[i].base_freq = dma_snd_freqs[ch_sfx[i].base_note];
							ch_sfx[i].current_freq = dma_snd_freqs[ch_sfx[i].current_note];
						} else {
							ch_sfx[i].base_freq = SNOI_POLYNOM(ch_sfx[i].base_note);
							ch_sfx[i].current_freq = SNOI_POLYNOM(ch_sfx[i].current_note);
						}
						ch_sfx[i].queue_note = 1;
						ch_sfx[i].delay_enabled = 0;
					}
				} else if (ch_sfx[i].portamento_direction) {
					//portamento up
					if (ch_sfx[i].portamento_direction == 1)
						ch_sfx[i].current_freq += ch_sfx[i].portamento_speed;
					//portamento down
					else if (ch_sfx[i].portamento_direction == 2)
						ch_sfx[i].current_freq -= ch_sfx[i].portamento_speed;
					//tone portamento
					else if (ch_sfx[i].portamento_direction == 3) {
						if (ch_sfx[i].portamento_freq_goal > ch_sfx[i].current_freq)
							ch_sfx[i].current_freq -= ch_sfx[i].portamento_speed;
						else if (ch_sfx[i].portamento_freq_goal < ch_sfx[i].current_freq)
							ch_sfx[i].current_freq += ch_sfx[i].portamento_speed;
					}
				} else if (ch_sfx[i].arp_enabled) {
					switch (advik_arp_index) {
						case 1:
							ch_sfx[i].current_note = ch_sfx[i].base_note + (ch_sfx[i].arp_offsets >> 4);
							break;
						case 2:
							ch_sfx[i].current_note = ch_sfx[i].base_note + (ch_sfx[i].arp_offsets & 0x0f);
							break;
						default:
							ch_sfx[i].current_note = ch_sfx[i].base_note;
					}
					if (i < 3) {
						...
						ch_sfx[i].base_freq = SNOI_POLYNOM(snd_freqs[ch_sfx[i].base_note]);
						ch_sfx[i].current_freq = SNOI_POLYNOM(snd_freqs[ch_sfx[i].current_note]);
					}
				} else if (ch_sfx[i].cut_delay_enabled) {
					if (ch_sfx[i].delay_ticks_counter > 0) {
						ch_sfx[i].delay_ticks_counter--;
					} else {
						ch_sfx[i].queue_cut = 1;
						ch_sfx[i].cut_delay_enabled = 0;
					}
				} else if (ch_sfx[i].vib_enabled) {
					ch_sfx[i].vib_ticks_counter += ch_sfx[i].vib_speed;
					ch_sfx[i].vib_position = ch_sfx[i].vib_ticks_counter & 63;
					ch_sfx[i].current_freq = (ch_sfx[i].base_freq - ((vib_table[ch_sfx[i].vib_position] * (ch_sfx[i].vib_depth)) >> 5));
				} else if (ch_sfx[i].volume_slide_enabled) {
					//if volume is greater than 0
					if ((ch_sfx[i].volume & 0x0f)) {
						//clamp it to 0 if speed is too fast and negative
						if (ch_sfx[i].volume_slide_speed >= (ch_sfx[i].volume & 0xfff0))
							ch_sfx[i].volume = 0;
						//or clamp it to 15 if speed is too fast and positive
						else if (ch_sfx[i].volume_slide_speed <= -(ch_sfx[i].volume & 0xfff0))
							ch_sfx[i].volume = (ch_sfx[i].volume & 0xfff0) | 0x0F;
						//or decrease the volume by the slide speed
						else
							ch_sfx[i].volume -= ch_sfx[i].volume_slide_speed;
						ch_sfx[i].queue_volume = 1;
					}
				}
			
		} else {
				if (!advik_ch_sfx_command_linger[i] && !advik_ch_sfx_override[i]) {
					ch_sfx[i].arp_enabled = 0;
					ch_sfx[i].portamento_direction = 0;
					ch_sfx[i].vib_enabled = 0;
					ch_sfx[i].vib_ticks_counter = 0;
					ch_sfx[i].current_note = ch_sfx[i].base_note;
					ch_sfx[i].volume = 0x0f;
				}
			
				if(advik_ch_sfx_override[i]){
					ch_sfx[i].effect = advik_ch_sfx_order[i][advik_ch_sfx_current[i]+2];
					ch_sfx[i].effect_param = advik_ch_sfx_order[i][advik_ch_sfx_current[i]+3];
					
					switch (ch_sfx[i].effect) {
						//if arpeggio
						case 0x00:
							if (ch_sfx[i].effect_param > 0) {
								ch_sfx[i].arp_offsets = ch_sfx[i].effect_param;
								ch_sfx[i].arp_enabled = 1;
							} else if (ch_sfx[i].effect_param == 0 && advik_ch_sfx_command_linger[i]) {
								ch_sfx[i].arp_enabled = 0;
								ch_sfx[i].current_note = ch_sfx[i].base_note;
							}
							break;
						...
						case 0x01:
							if (ch_sfx[i].effect_param > 0) {
								ch_sfx[i].portamento_direction = 1;
								ch_sfx[i].portamento_speed = ch_sfx[i].effect_param;
							} else if (ch_sfx[i].effect_param == 0 && advik_ch_sfx_command_linger[i]) {
								ch_sfx[i].portamento_direction = 0;
							}
							break;
						...
						case 0x02:
							if (ch_sfx[i].effect_param > 0) {
								ch_sfx[i].portamento_direction = 2;
								ch_sfx[i].portamento_speed = ch_sfx[i].effect_param;
							} else if (ch_sfx[i].effect_param == 0 && advik_ch_sfx_command_linger[i]) {
								ch_sfx[i].portamento_direction = 0;
							}
						...
						case 0x03:
							if (ch_sfx[i].effect_param > 0) {
								ch_sfx[i].portamento_direction = 3;
								if (advik_ch_current_order[i][advik_current] < 0xf0) {
									ch_sfx[i].portamento_note_goal = advik_ch_sfx_order[i][advik_ch_sfx_current[i]];
									if (i < 3)
										ch_sfx[i].portamento_freq_goal = snd_freqs[advik_ch_sfx_order[i][advik_ch_sfx_current[i]]];
									else if (i >= 4)
										ch_sfx[i].portamento_freq_goal = dma_snd_freqs[advik_ch_sfx_order[i][advik_ch_sfx_current[i]]];
								}
							} else if (ch_sfx[i].effect_param == 0 && advik_ch_sfx_command_linger[i]) {
								ch_sfx[i].portamento_direction = 0;
							}
						...
						//if vibrato
						case 0x04:
							ch_sfx[i].vib_depth = ch_sfx[i].effect_param & 0x0f;
							ch_sfx[i].vib_speed = ch_sfx[i].effect_param & 0xf0;
							ch_sfx[i].vib_enabled = 1;
							if (ch_sfx[i].effect_param == 0 && advik_ch_sfx_command_linger[i]) {
								ch_sfx[i].vib_enabled = 0;
								ch_sfx[i].current_note = ch_sfx[i].base_note;
							}
							break;
						//if set master volume
						case 0x05:
							ch_sfx[i].queue_global_volume = 1;
							if (i < 4)
								advik_global_volume = ch_sfx[i].effect_param;
							break;
						//if note delay
						case 0x07:
							ch_sfx[i].delay_enabled = 1;
							ch_sfx[i].delay_ticks_counter = ch_sfx[i].effect_param - 1;
							break;
						//if set global panning
						case 0x08:
							ch_sfx[i].queue_global_pan = 1;
							break;
						//if change timbre
						case 0x09:
							ch_sfx[i].timbre_goal = ch_sfx[i].effect_param;
							ch_sfx[i].queue_timbre = 1;
							break;
						//if volume slide
						case 0x0a:
							if (ch_sfx[i].effect_param == 0 && advik_ch_sfx_command_linger[i]) {
								ch_sfx[i].volume_slide_enabled = 0;
							} else {
								if ((ch_sfx[i].effect_param & 0xf0) > 0)
									ch_sfx[i].volume_slide_speed = -ch_sfx[i].effect_param;
								else if ((ch_sfx[i].effect_param & 0x0f) > 0)
									ch_sfx[i].volume_slide_speed = ch_sfx[i].effect_param;
								ch_sfx[i].volume_slide_enabled = 1;
							}
							break;
						//if set volume
						case 0x0c:
							ch_sfx[i].queue_volume = 1;
							ch_sfx[i].volume = ch_sfx[i].effect_param;
							break;
						//if pattern break
						case 0x0d:
							advik_ch_sfx_override[i] = 0;
						...
						//if note cut
						case 0x0e:
							ch_sfx[i].cut_delay_enabled = 1;
							ch_sfx[i].delay_ticks_counter = ch_sfx[i].effect_param - 1;
							break;
						//if set tempo
						case 0x0f:
							advik_ticks_per_row_value = ch_sfx[i].effect_param - 1;
							break;
						//if set furnace panning
						case 0x80:
							ch_sfx[i].pan = ch_sfx[i].effect_param;
					}
					if (advik_ch_sfx_order[i][advik_ch_sfx_current[i]] < 0xf8 && !ch_sfx[i].delay_enabled) {
						//if instrument isn't 0, retrigger note
						if (advik_ch_sfx_order[i][advik_ch_sfx_current[i]+1] > 0x00){
							ch_sfx[i].instrument = advik_ch_sfx_order[i][advik_ch_sfx_current[i]+1] - 1;
							ch_sfx[i].queue_instrument = 1;
						}
						
						ch_sfx[i].base_note = advik_ch_sfx_order[i][advik_ch_sfx_current[i]];
						ch_sfx[i].current_note = ch_sfx[i].base_note;
						if (i < 3) {
							ch_sfx[i].base_freq = snd_freqs[ch_sfx[i].base_note];
							ch_sfx[i].current_freq = snd_freqs[ch_sfx[i].current_note];
							...
							ch_sfx[i].base_freq = dma_snd_freqs[ch_sfx[i].base_note];
							ch_sfx[i].current_freq = dma_snd_freqs[ch_sfx[i].current_note];
							ch_sfx[i].sample_duration = advik_sfx_current_dma_durations[i - 4][ch_sfx[i].instrument];
						} else {
							ch_sfx[i].base_freq = SNOI_POLYNOM(ch_sfx[i].base_note);
							ch_sfx[i].current_freq = SNOI_POLYNOM(ch_sfx[i].current_note);
						}
	...
	if (ch_sfx[4].sample_duration){
		ch_sfx[4].sample_duration--;
	} else {
		ch_sfx[4].queue_cut = 1;
	}
	if (ch_sfx[5].sample_duration){
		ch_sfx[5].sample_duration--;
	} else {
		ch_sfx[5].queue_cut = 1;
	}
	
	if (advik_ch_sfx_override[0])
		cell_song_hw_write_sfx_ch1(advik_sfx_current_square_instruments[0]);
	if (advik_ch_sfx_override[1])
		cell_song_hw_write_sfx_ch2(advik_sfx_current_square_instruments[1]);
	if (advik_ch_sfx_override[2])
		cell_song_hw_write_sfx_ch3(advik_sfx_current_wave_instruments, advik_sfx_current_waveforms);
	if (advik_ch_sfx_override[3])
		cell_song_hw_write_sfx_ch4(advik_sfx_current_noise_instruments);
	if (advik_ch_sfx_override[4])
		cell_song_hw_write_sfx_ch5(advik_sfx_current_dma_instruments[0]);
	if (advik_ch_sfx_override[5])
		cell_song_hw_write_sfx_ch6(advik_sfx_current_dma_instruments[1]);
}